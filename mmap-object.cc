...
NAN_METHOD(SharedMapControl::Close) {
  ...
  if (!self)
    return;
  LOCKINFO("CLOSE");
  bip::scoped_lock<upgradable_mutex_type> lock(self->map->mutexes->global_mutex);
  BOOST_SCOPE_EXIT(self) {
    UNLOCKINFO("CLOSE");
  } BOOST_SCOPE_EXIT_END;
  ...
  if (self->map->writeonly) {
  //self->map->map_seg->get_segment_manager()->shrink_to_fit_indexes();
    bip::managed_mapped_file::shrink_to_fit(self->map->file_name.c_str());
  }
  self->map->map_seg->flush();
  delete self->map->map_seg;
  self->map->map_seg = NULL;
  self->map->closed = true;
  // Release WO lock.
  if (self->map->writeonly) {
    self->map->mutexes->wo_mutex.unlock();
  } else {
    self->map->mutexes->wo_mutex.unlock_sharable();
  }
}
